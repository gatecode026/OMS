...
const Reports = () => {
  ...
  const branchPerfData = useMemo(() => {
    ...
    return months.map((month, mIdx) => {
      const row = { month };
      (branches || []).forEach(b => {
        const branchName = b.name.toLowerCase().replace(/\s+branch/g, '').trim();
        const branchEmps = (employees || []).filter(e => e.branch === b.name);
        const currentProd = branchEmps.length > 0 ? Math.round(branchEmps.reduce((sum, e) => sum + (e.productivityScore ?? 0), 0) / branchEmps.length) : (b.productivity || 90);
        const variation = (mIdx - 5) * 1;
        row[branchName] = Math.max(70, Math.min(100, currentProd + variation));
      });
      return row;
    });
  }, [branches, employees]);
  ...
  const branchRadarData = useMemo(() => {
    const metrics = ['Productivity', 'Attendance', 'Performance'];
    return metrics.map(metric => {
      ...
      (branches || []).forEach(b => {
        ...
        if (metric === 'Productivity') {
          val = branchEmps.length > 0 ? Math.round(branchEmps.reduce((sum, e) => sum + (e.productivityScore ?? 0), 0) / branchEmps.length) : (b.productivity || 90);
        } else if (metric === 'Attendance') {
          val = branchEmps.length > 0 ? Math.round(branchEmps.reduce((sum, e) => sum + (e.attendanceStatus === 'Present' || e.attendanceStatus === 'Late' ? 95 : 90), 0) / branchEmps.length) : 94;
        } else if (metric === 'Performance') {
          val = branchEmps.length > 0 ? Math.round(branchEmps.reduce((sum, e) => sum + (e.performanceScore?.overall ?? 0), 0) / branchEmps.length) : 85;
        }
        row[branchName] = val;
      });
      return row;
  ...
  // Compute Branch Salary Cost Comparison Data dynamically
  const branchSalaryData = useMemo(() => {
    return (branches || []).map(b => {
      const branchEmps = (employees || []).filter(e => e.branch === b.name);
      const cost = branchEmps.reduce((sum, e) => {
        const sal = e.salary || (e.experience * 15000 + 40000);
        return sum + sal;
      }, 0);
      return {
        branch: b.name,
  ...
  const handleDownloadReport = (report) => {
    addPageToast('info', `Downloading ${report.name}...`);
    ...
    if (type === 'Attendance') {
      headers = ['Date', 'Employee ID', 'Employee Name', 'Status', 'Punch In', 'Punch Out', 'Work Hours'];
      rows = (attendance || []).map(a => [
        ...
        a.punchIn || '',
        a.punchOut || '',
        a.totalHours ? `${a.totalHours} hrs` : '0 hrs'
      ]);
    } else if (type === 'Payroll') {
      headers = ['Employee ID', 'Employee Name', 'Department', 'Basic Salary', 'Gross Salary', 'Total Deductions', 'Net Take-home', 'Payment Status'];
    ...
    } else if (type === 'Projects' || type === 'Branch') {
      if (type === 'Projects') {
        headers = ['Project Name', 'Department', 'Branch', 'Progress (%)', 'Status'];
      ...
      headers = ['Employee Name', 'Leave Type', 'From Date', 'To Date', 'Status', 'Reason'];
    ...
    if (format === 'PDF') {
      ...
      if (printWindow) {
        ...
        printWindow.document.write(`
          ...
          <html>
            ...
            <body>
              <h1>${report.name}</h1>
              <p>Generated by: ${report.generatedBy} | Time: ${report.time}<br/>Generated on: ${new Date().toLocaleString()}</p>
              <table>
      ...
      a.href = url;
      a.download = `${report.name.replace(/\s+/g, '_')}_${Date.now()}.${format === 'Excel' ? 'xls' : 'csv'}`;
  ...
  const handleExport = (e) => {
    ...
    // Export general executive dashboard summary
    const headers = ['Executive KPI Metric', 'Value'];
    const rows = [
      ['Total Reports Generated', '25,480'],
      ['Reports Today', '128'],
      ['Scheduled Reports', '42'],
      ['Active Dashboards', '18'],
      ['Productivity Score', '92%'],
      ['Business Growth Index', '18.5%'],
      ['Active Reports', '156']
    ];
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `executive_analytics_summary_${Date.now()}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    addPageToast('success', 'Executive analytics exported successfully.');
  };
  ...
  const renderReportTable = (reportNames, addPageToast, extraHeaders = []) => {
    const handleDownloadReportByName = (name) => {
      addPageToast('success', `Downloading ${name}...`);
      ...
      if (lowerName.includes('employee') || lowerName.includes('defaulter') || lowerName.includes('resource')) {
        matched = true;
        headers = ['Employee ID', 'Employee Name', 'Role/Designation', 'Department', 'Branch', 'Productivity Score (%)', 'Status'];
        rows = (employees || []).map(e => [
          e.id || '',
      ...
      } else if (lowerName.includes('leave')) {
        matched = true;
        headers = ['Employee Name', 'Leave Type', 'From Date', 'To Date', 'Status', 'Reason'];
        rows = (leaveRequests || []).map(l => [
          l.employeeName || '',
      ...
      } else if (lowerName.includes('payroll') || lowerName.includes('salary') || lowerName.includes('revision') || lowerName.includes('bonus') || lowerName.includes('pf') || lowerName.includes('esi') || lowerName.includes('tds') || lowerName.includes('tax')) {
        matched = true;
        headers = ['Employee ID', 'Employee Name', 'Department', 'Basic Salary', 'Gross Salary', 'Total Deductions', 'Net Take-home', 'Payment Status'];
        rows = filteredData.map(p => [
          p.employeeId || '',
      ...
      } else if (lowerName.includes('project') || lowerName.includes('milestone')) {
        matched = true;
        headers = ['Project Name', 'Department', 'Branch', 'Progress (%)', 'Status'];
        rows = (projectsList || []).map(p => [
          p.name || '',
        ...
        matched = true;
        headers = ['Task ID', 'Title', 'Project', 'Assignee Name', 'Department', 'Due Date', 'Priority', 'Progress (%)', 'Status'];
        rows = (tasks || []).map(t => [
          t.id || '',
          t.title || '',
          t.project || '',
        ...
        matched = true;
        headers = ['Department Name', 'Branch/Location', 'Workforce Count', 'Status'];
        rows = (rawDepartments || []).map(d => [
          d.name || '',
          d.branch || '',
          (employees || []).filter(e => e.department === d.name && e.status !== 'Inactive').length,
          d.status || 'Active'
        ]);
      ...
      if (!matched) {
        const content = `Report: ${name}\nGenerated: ${new Date().toLocaleString()}\n\nThis is a compiled report representing OMS system database snapshot.`;
        const blob = new Blob([content], { type: 'text/plain;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `${name.replace(/\s+/g, '_')}_${Date.now()}.txt`;
        a.click();
        return;
      }
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `${name.replace(/\s+/g, '_')}_${Date.now()}.csv`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
                ...
                <tr key={i}>
                  ...
                  <td>
                    <div className="rpt-row-actions" style={{ justifyContent: 'flex-end' }}>
                      <button className="rpt-action-btn" title="Download" onClick={() => handleDownloadReportByName(name)}><Download size={13} /></button>
                      <button className="rpt-action-btn" title="Preview" onClick={() => handleDownloadReportByName(name)}><Eye size={13} /></button>
                      <button className="rpt-action-btn" title="Email" onClick={() => addPageToast('info', `Report ${name} sent to your email.`)}><Mail size={13} /></button>
                    </div>
                  </td>
                </tr>
              ))}
  ...
  return (
    <div className="rpt-page flex-column grid-gap">
      ...
      <div className="rpt-stats-grid">
        <StatCard label="Total Reports Generated" value={reports.length} trend="+8.2%" trendUp={true} subtitle="All time" colorClass="border-bottom-primary" icon={FileText} />
        <StatCard label="Reports Today" value={reports.filter(r => r.time === 'Just now' || r.time.includes('min') || r.time.includes('hour')).length} trend="+15%" trendUp={true} subtitle="vs yesterday" colorClass="border-bottom-success" icon={Calendar} />
        <StatCard label="Scheduled Reports" value={schedules.length} trend="+4" trendUp={true} subtitle="Active schedules" colorClass="border-bottom-info" icon={Clock} />
        <StatCard label="Active Dashboards" value="8" trend="+2" trendUp={true} subtitle="Live analytics" colorClass="border-bottom-primary" icon={LayoutDashboard} />
        <StatCard label="Productivity Score" value={`${Math.round((employees || []).reduce((sum, e) => sum + (e.productivityScore || 0), 0) / (employees?.length || 1))}%`} trend="+3.5%" trendUp={true} subtitle="vs last quarter" colorClass="border-bottom-success" icon={TrendingUp} />
        ...
        <StatCard label="Active Reports" value={reports.length} trend="+12" trendUp={true} subtitle="Running now" colorClass="border-bottom-info" icon={Activity} />
        <StatCard label="Report Categories" value="8" trend="0" trendUp={true} subtitle="Modules covered" colorClass="border-bottom-warning" icon={Layers} />
      </div>
