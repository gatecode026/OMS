...
const HiddenChats = ({ currentUser, onBack, onSelectConversation }) => {
  const {
    hiddenConversations,
  ...
  return (
    <div className="conv-list-container" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="conv-list-header" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px' }}>
        ...
        <h2 className="conv-list-title" style={{ margin: 0, fontSize: '18px', fontWeight: '700' }}>Hidden Chats</h2>
      </div>
      ...
      <div className="conv-list-items" style={{ flex: 1, overflowY: 'auto' }}>
        {hiddenConversations.length === 0 ? (
          <div style={{
            ...
            <span style={{ fontSize: '36px', display: 'block', marginBottom: '10px' }}>👁️‍🗨️</span>
            No hidden chats.
          </div>
        ) : (
          hiddenConversations.map(conv => {
            ...
            return (
              <ConversationItem
                ...
                isHiddenView={true}
                onUnhide={() => unhideConversation?.(conv.id)}
              />
            );
          })
        )}
